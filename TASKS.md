...
## TASK-OBS-001: Arricchire /health con memory_mb e latency_ms
**Assignee:** Thomas  
**Priority:** HIGH  
**Description:**
- Aggiungere `memory_mb` usando psutil
- Aggiungere `latency_ms` misurando round-trip Telegram API
- Aggiungere `db_ok` check (connessione al DB)
- Response 503 se degraded

**Acceptance Criteria:**
- `/health` ritorna JSON con tutti i campi nuovi
- Response time < 100ms

---

## TASK-OBS-002: Arricchire /stats con error_rate e last_alert_at
**Assignee:** Thomas  
**Priority:** HIGH  
**Description:**
- Aggiungere `error_rate` calcolato da AlertLogs
- Aggiungere `last_alert_at` (timestamp ultimo alert)
- Aggiungere `alerts_failed` count

**Acceptance Criteria:**
- `/stats` ritorna JSON con tutti i campi nuovi
- Error rate calcolato correttamente

---

## TASK-OBS-003: Integrare structlog per logging JSON
**Assignee:** Thomas  
**Priority:** HIGH  
**Description:**
- Installare structlog
- Sostituire print/log base con logging strutturato
- Campi: timestamp, level, message, user_id, command, latency_ms, request_id

**Acceptance Criteria:**
- Tutti i log in formato JSON
- Campi tracciabili presenti

---

## TASK-OBS-004: Setup Grafana Dashboard (v2)
**Assignee:** Piotr  
**Priority:** MEDIUM  
**Description:**
- Setup Grafana (hosting low-cost)
- Importare dashboard predefinita
- Connect to Prometheus (future)
...
## TASK-OBS-005: Definire SLO/SLA + Load Test Basic
**Assignee:** Goksu  
**Priority:** HIGH  
**Description:**
- Formalizzare SLO: uptime > 99.5%, latency < 500ms
- Definire load test basic: quanti msg/sec regge il bot prima di degradare
- Creare test plan documentato

**Acceptance Criteria:**
- SLO documentati e testabili
- Load test eseguito e risultati registrati
- Bottleneck identificati (se presenti)

---

## TASK-OBS-006: QA Checklist Validation
**Assignee:** Goksu  
**Priority:** HIGH  
**Dependencies:** TASK-OBS-001, TASK-OBS-002, TASK-OBS-005  
**Description:**
- Validare i 3 must-have QA:
  - /health → 200 OK, JSON with uptime/memory/db_status, latency < 500ms
  - /stats → 200 OK, JSON with error_count/avg_latency/delivery_success_rate
  - Error tracking → logs with traceback
- Load test results validate against SLO

**Acceptance Criteria:**
- Tutti i QA criteria passano
- Report con steps to reproduce per eventuali failures
